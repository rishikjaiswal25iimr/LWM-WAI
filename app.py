...
app.py
======
XpressBees Logistics Intelligence System — Executive Control Tower
...
st.set_page_config(
    page_title="XpressBees Logistics Intelligence System",
    page_icon="🚚",
    layout="wide",
...
@st.cache_data(show_spinner="Loading and preparing XpressBees dataset...")
...
<div class="control-tower-header">
    <h1>🚚 XpressBees Logistics Intelligence System</h1>
...
with st.sidebar:
    ...
    if opts["min_date"] is not None and pd.notna(opts["min_date"]):
        st.markdown(
            '<div style="color:#0B2545;background:#E7EEF7;display:inline-block;'
            'padding:0.15rem 0.55rem;border-radius:4px;font-weight:700;'
            'font-size:0.85rem;margin-bottom:0.3rem;">Order Date Range</div>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown("---")
    st.caption(
        "**Data provenance:** Original ~24 XpressBees operational fields are public-dataset "
        "based. Cost, fuel, CO₂, workload and risk fields are synthetic/derived modelling "
        "estimates for this WAI assignment — not audited XpressBees figures."
    )
...
def risk_bar_chart(df_in, groupby_col, title, top_n=12, chart_key=None):
    ...
    key = chart_key or f"risk_bar_{groupby_col}_{title}"
    st.plotly_chart(fig, width='stretch', key=key)


def _find_col(columns, keywords):
    """Best-effort, case-insensitive column lookup so the Managerial Decision
    Engine tab can render dynamically whatever schema core.py returns, without
    hardcoding column names. Used only by that tab."""
    cols = list(columns)
    for kw in keywords:
        for c in cols:
            if kw.lower() == c.lower():
...
def chip_row(items, color=ACCENT):
    """Render a wrapped row of small colored badge 'chips' — a compact,
    visual stand-in for comma-separated lists of field names."""
    items = [str(i) for i in items if str(i).strip()]
    if not items:
        return
    chips = "".join(
        f'<span style="background:{color}1A;color:{color};border:1px solid {color}66;'
        f'padding:0.22rem 0.65rem;border-radius:12px;font-size:0.78rem;margin:0.15rem;'
...
def flow_diagram(steps, box_color=PRIMARY, text_color="white", per_row=None):
    """Render a horizontal box → box → box flow diagram using plain
    Streamlit columns + HTML, wrapping to multiple rows if per_row is set."""
    ...
    def _row(chunk):
        ...
        for i in range(n):
            ...
            if i < n - 1:
                widths.append(0.55)
        flow_cols = st.columns(widths)
        idx = 0
        for i, step in enumerate(chunk):
            step_html = str(step).replace("\n", "<br>")
            with flow_cols[idx]:
                st.markdown(
                    f"<div style='text-align:center;background:{box_color};color:{text_color};"
                    f"padding:0.55rem 0.4rem;border-radius:8px;font-weight:650;font-size:0.78rem;"
                    f"box-shadow:0 1px 3px rgba(11,37,69,0.18);min-height:2.6rem;"
                    f"display:flex;align-items:center;justify-content:center;line-height:1.3;'>{step_html}</div>",
                    unsafe_allow_html=True,
                )
            idx += 1
            if i < n - 1:
                with flow_cols[idx]:
                    st.markdown(
                        f"<div style='text-align:center;font-size:1.2rem;color:{ACCENT};"
                        f"padding-top:0.35rem;'>→</div>",
                        unsafe_allow_html=True,
                    )
                idx += 1

    if per_row and len(steps) > per_row:
        for start in range(0, len(steps), per_row):
            chunk = steps[start:start + per_row]
            _row(chunk)
            if start + per_row < len(steps):
                st.markdown(
                    f"<div style='text-align:center;font-size:1.1rem;color:{ACCENT};"
                    f"margin:-0.15rem 0 0.15rem 0;'>↓</div>",
                    unsafe_allow_html=True,
                )
    else:
        _row(steps)
...
with tabs[0]:
    ...
    with col_a:
        ...
        if "daily_trend" in desc and len(desc["daily_trend"]) > 1:
            ...
            st.plotly_chart(fig, width='stretch', key=f"chart_2")
        else:
    ...
    with col_b:
        ...
        if "risk_level" in df.columns:
            ...
            st.plotly_chart(fig, width='stretch', key=f"chart_3")

    col_c, col_d = st.columns(2)
    with col_c:
        subsection("Hub Performance (by Source Hub)")
        risk_bar_chart(df, "source_name", "High-Risk Trip % by Source Hub (Top 12)", chart_key="ect_hub")
    with col_d:
        subsection("Route Performance")
        risk_bar_chart(df, "route_label", "High-Risk Trip % by Route (Top 12)", chart_key="ect_route")
    ...
    with col_e:
        ...
        if {"cost_per_km", "delayed_flag", "source_name"}.issubset(df.columns):
            ...
            st.plotly_chart(fig, width='stretch', key=f"chart_4")
    with col_f:
...
with tabs[1]:
    ...
    with col_a:
        ...
        if "by_route" in desc:
            ...
            st.plotly_chart(fig, width='stretch', key=f"chart_6")
    with col_b:
        ...
        if {"fuel_cost_inr", "toll_cost_inr", "estimated_delay_cost_inr"}.issubset(df.columns):
            ...
            st.plotly_chart(fig, width='stretch', key=f"chart_7")
    ...
    with col_c:
        ...
        if "by_vehicle_type" in sustain:
            ...
            st.plotly_chart(fig, width='stretch', key=f"chart_8")
    with col_d:
        ...
        if {"cost_per_km", "delayed_flag", "route_label"}.issubset(df.columns):
            ...
            fig = px.scatter(route_matrix, x="avg_cost_per_km", y="delay_pct", size="trips",
                            ...
                            color="delay_pct", color_continuous_scale=["#2E8B57", "#E6A817", "#C21B17"])
            fig.update_layout(height=380, coloraxis_showscale=False)
            st.plotly_chart(fig, width='stretch', key=f"chart_9")
    ...
    if {"cost_per_km", "route_label"}.issubset(df.columns):
        ...
        if len(hc_hr):
            st.dataframe(hc_hr, width='stretch', height=280)
        else:
...
with tabs[2]:
    ...
    st.info("⚠️ Scenarios below are **illustrative modelled simulations** for MBA decision-support purposes. "
            "They do not represent an actual XpressBees operational commitment.", icon="ℹ️")
    ...
    st.plotly_chart(fig, width='stretch', key=f"chart_10")
...
with tabs[3]:
    ...
    if ml_pipeline["classification"].get("status") != "ok":
        ...
        with col_a:
            ...
            fig.update_layout(height=380)
            st.plotly_chart(fig, width='stretch', key=f"chart_11")
        with col_b:
            ...
            if "route_type" in df.columns:
                ...
                fig.update_layout(height=380)
                st.plotly_chart(fig, width='stretch', key=f"chart_12")

        col_c, col_d = st.columns(2)
        with col_c:
            subsection("High-Risk Route Ranking")
            risk_bar_chart(df, "route_label", "High-Risk Trip % by Route (Top 12)", chart_key="risk_tab_route")
        with col_d:
            subsection("High-Risk Hub Ranking")
            risk_bar_chart(df, "source_name", "High-Risk Trip % by Source Hub (Top 12)", chart_key="risk_tab_hub")
        ...
        fig.update_layout(height=420, yaxis={"categoryorder": "total ascending"})
        st.plotly_chart(fig, width='stretch', key=f"chart_13")
...
with tabs[4]:
    ...
    with col_a:
        ...
        if {"actual_distance_to_destination", "osrm_distance"}.issubset(df.columns):
            ...
            st.plotly_chart(fig, width='stretch', key=f"chart_14")
    with col_b:
        ...
        if {"actual_time", "osrm_time"}.issubset(df.columns):
            ...
            fig.add_trace(go.Scatter(x=[0, max_v], y=[0, max_v], mode="lines", line=dict(dash="dash", color="gray"), name="Perfect match"))
            fig.update_layout(height=400)
            st.plotly_chart(fig, width='stretch', key=f"chart_15")
    ...
    with col_c:
        ...
        if "route_efficiency_pct" in df.columns:
            ...
            fig.update_layout(height=380)
            st.plotly_chart(fig, width='stretch', key=f"chart_16")
    with col_d:
        ...
        if "by_route_type" in desc:
            fig = px.bar(desc["by_route_type"], x="route_type", y="avg_delay", title="Avg Delay (min) by Route Type",
                        color_discrete_sequence=[ACCENT])
            fig.update_layout(height=380)
            st.plotly_chart(fig, width='stretch', key=f"chart_17")
    ...
    if "by_route" in desc:
        ...
        st.dataframe(inefficient, width='stretch', height=380)
...
with tabs[5]:
    ...
    with col_a:
        ...
        if "by_state" in desc:
            ...
            fig.update_layout(height=380)
            st.plotly_chart(fig, width='stretch', key=f"chart_18")
    with col_b:
        ...
        if "by_vehicle_type" in desc:
            ...
            fig.update_layout(height=380)
            st.plotly_chart(fig, width='stretch', key=f"chart_19")
    ...
    with col_c:
        ...
        if "by_source_hub" in desc:
            st.dataframe(desc["by_source_hub"].head(15), width='stretch', height=340)
    with col_d:
        subsection("Destination Hub Performance")
        if "by_destination_hub" in desc:
            st.dataframe(desc["by_destination_hub"].head(15), width='stretch', height=340)
...
with tabs[6]:
    ...
    with col_a:
        ...
        if "numeric_correlations" in diag:
            ...
            st.plotly_chart(fig, width='stretch', key=f"chart_20")
    ...
    with col_b:
        ...
        if "categorical_effects" in diag:
            ...
            fig.update_layout(height=420, yaxis={"categoryorder": "total ascending"})
            st.plotly_chart(fig, width='stretch', key=f"chart_21")

    subsection("Delayed vs On-Time Trip Profile")
    if "delayed_vs_ontime_profile" in diag:
        st.dataframe(diag["delayed_vs_ontime_profile"], width='stretch', hide_index=True)
    ...
    with col_c:
        ...
        if {"traffic_level", "delayed_flag"}.issubset(df.columns):
            ...
            st.plotly_chart(fig, width='stretch', key=f"chart_22")
    with col_d:
        ...
        if {"weather_condition", "delayed_flag"}.issubset(df.columns):
            ...
            fig.update_layout(height=380, showlegend=False)
            st.plotly_chart(fig, width='stretch', key=f"chart_23")
    ...
    if {"hub_workload_pct", "time_deviation_minutes"}.issubset(df.columns):
        ...
        st.plotly_chart(fig, width='stretch', key=f"chart_24")
...
with tabs[7]:
    ...
    with ml_tab1:
        if clf.get("status") != "ok":
            ...
            st.dataframe(clf["model_comparison"], width='stretch', hide_index=True)
            ...
            with col_a:
                ...
                fig.update_layout(height=380)
                st.plotly_chart(fig, width='stretch', key=f"chart_25")
            with col_b:
                ...
                st.plotly_chart(fig, width='stretch', key=f"chart_26")
    ...
    with ml_tab2:
        if reg.get("status") != "ok":
            ...
            st.dataframe(reg["model_comparison"], width='stretch', hide_index=True)
            ...
            fig.update_layout(height=420)
            st.plotly_chart(fig, width='stretch', key=f"chart_27")
    ...
    with ml_tab3:
        if clus.get("status") != "ok":
            ...
            st.dataframe(clus["silhouette_scores"], width='stretch', hide_index=True)
            ...
            if "cluster_segment" in ml_pipeline["scored_df"].columns:
                ...
                fig = px.pie(seg_counts, names="Segment", values="Trips", title="Trip Segments (Full Dataset)",
                            hole=0.4)
                fig.update_layout(height=400)
                st.plotly_chart(fig, width='stretch', key=f"chart_28")
    ...
    with ml_tab4:
        ...
        if clf.get("status") == "ok":
            ...
            st.plotly_chart(fig, width='stretch', key=f"chart_29")
        ...
        if reg.get("status") == "ok":
            ...
            fig.update_layout(height=460, yaxis={"categoryorder": "total ascending"})
            st.plotly_chart(fig, width='stretch', key=f"chart_30")
        st.markdown("""
...
with tabs[8]:
    ...
    st.caption("🔄 Auto-generated from the current filtered dataset — nothing below is hardcoded.")

    recs = core.generate_managerial_recommendations(df)

    # Dynamic schema detection — resilient to whatever columns core.py returns.
    rec_cols = list(recs.columns)
    priority_col = _find_col(rec_cols, ["priority"])
    area_col = _find_col(rec_cols, ["area", "category", "domain"])
    problem_col = _find_col(rec_cols, ["issue", "problem"])
    evidence_col = _find_col(rec_cols, ["evidence"])
    action_col = _find_col(rec_cols, ["recommended action", "action", "recommendation"])
    impact_col = _find_col(rec_cols, ["expected impact", "impact", "objective", "outcome"])

    PRIORITY_COLORS = {"P1": NEGATIVE, "P2": WARNING, "P3": ACCENT}

    if recs.empty:
        st.success("✅ No material issues detected for the current filter selection — all monitored "
                    "KPIs are within normal thresholds.")
    else:
        # ---- KPI strip -----------------------------------------------------
        n_total = len(recs)
        n_p1 = int((recs[priority_col] == "P1").sum()) if priority_col else 0
        n_p2 = int((recs[priority_col] == "P2").sum()) if priority_col else 0
        n_p3 = int((recs[priority_col] == "P3").sum()) if priority_col else 0

        c1, c2, c3, c4 = st.columns(4)
        kpi_card(c1, "Total Recommendations", f"{n_total}")
        kpi_card(c2, "🔴 P1 — Urgent", f"{n_p1}", sub="Risk / operational")
        kpi_card(c3, "🟠 P2 — Efficiency", f"{n_p2}", sub="Cost / efficiency")
        kpi_card(c4, "🔵 P3 — Structural", f"{n_p3}", sub="Network / structural")
        ...
        with col_left:
            subsection("Recommendations by Priority")
            if priority_col:
                pr_counts = (
                    recs[priority_col].value_counts()
                    .reindex(["P1", "P2", "P3"]).fillna(0).astype(int).reset_index()
                )
                pr_counts.columns = ["Priority", "Count"]
                fig = px.bar(pr_counts, x="Priority", y="Count", color="Priority",
                             color_discrete_map=PRIORITY_COLORS, title="Recommendation Volume by Priority")
                fig.update_layout(height=330, showlegend=False)
                st.plotly_chart(fig, width='stretch', key="mde_priority_bar")
        with col_right:
            subsection("Top Intervention Areas")
            if area_col:
                area_counts = recs[area_col].value_counts().head(8).reset_index()
                area_counts.columns = ["Area", "Count"]
                fig = px.bar(area_counts, x="Count", y="Area", orientation="h",
                             title="Recommendations by Business Area", color_discrete_sequence=[PRIMARY])
                fig.update_layout(height=330, yaxis={"categoryorder": "total ascending"})
                st.plotly_chart(fig, width='stretch', key="mde_area_bar")
            else:
                st.info("Area breakdown not available in the current recommendation schema.")

        # ---- Problem → Evidence → Recommendation → Expected Impact flow -----
        subsection("How Each Recommendation Is Built")
        flow_diagram(["🔍 Problem", "📊 Evidence", "💡 Recommendation", "🎯 Expected Impact"])
        st.caption("Every card below follows this flow — generated live from the current filters, "
                   "then use the Scenario Simulator tab to quantify the impact of acting on one.")
        ...
        if priority_col:
            display_recs["_sort"] = display_recs[priority_col].map({"P1": 0, "P2": 1, "P3": 2}).fillna(9)
            display_recs = display_recs.sort_values("_sort")

        MAX_CARDS = 12
        for _, row in display_recs.head(MAX_CARDS).iterrows():
            pr = row[priority_col] if priority_col else "—"
            color = PRIORITY_COLORS.get(pr, ACCENT)
            area_txt = row[area_col] if area_col else "General"
            problem_txt = row[problem_col] if problem_col else "—"
            evidence_txt = row[evidence_col] if evidence_col else None
            action_txt = row[action_col] if action_col else "—"
            impact_txt = row[impact_col] if impact_col else None

            evidence_html = (
                f"<div style='font-size:0.8rem;color:#5B6B82;margin-top:0.25rem;'>"
                f"<b>📊 Evidence:</b> {evidence_txt}</div>" if evidence_txt else ""
            )
            impact_html = (
                f"<div style='font-size:0.8rem;color:{POSITIVE};margin-top:0.25rem;'>"
                f"<b>🎯 Expected Impact:</b> {impact_txt}</div>" if impact_txt else ""
            )

            st.markdown(f"""
            <div style="background:white;border-left:6px solid {color};border-radius:8px;
                        padding:0.7rem 1.1rem;margin-bottom:0.6rem;box-shadow:0 1px 4px rgba(11,37,69,0.10);">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <span style="font-weight:700;color:{PRIMARY};">{area_txt}</span>
                    <span style="background:{color};color:white;padding:0.15rem 0.65rem;
                                border-radius:12px;font-size:0.72rem;font-weight:700;">{pr}</span>
                </div>
                <div style="font-size:0.85rem;margin-top:0.35rem;"><b>🔍 Problem:</b> {problem_txt}</div>
                {evidence_html}
                <div style="font-size:0.85rem;margin-top:0.25rem;color:{ACCENT};">
                    <b>💡 Recommendation:</b> {action_txt}</div>
                {impact_html}
            </div>
            """, unsafe_allow_html=True)

        if n_total > MAX_CARDS:
            st.caption(f"Showing top {MAX_CARDS} of {n_total} recommendations by priority — "
                       f"see the full table below for the rest.")

        with st.expander(f"📋 View all {n_total} recommendations as a table"):
            st.dataframe(recs, width='stretch', hide_index=True, height=360)
            st.download_button(
                "⬇️ Download Recommendations (CSV)",
                data=recs.to_csv(index=False).encode("utf-8"),
                file_name="xpressbees_managerial_recommendations.csv",
                mime="text/csv",
            )
...
with tabs[9]:
    ...
    flow_diagram(["📦 Public XpressBees\nDataset (~24 fields)", "➕ Synthetic / Derived\nEnrichment (WAI)",
                  "✅ Prepared Analytical\nDataset"])
    ORIGINAL_FIELD_ESTIMATE = 24
    synthetic_estimate = max(full_df.shape[1] - ORIGINAL_FIELD_ESTIMATE, 0)
    prov_col1, prov_col2 = st.columns([1, 1.3])
    with prov_col1:
        origin_df = pd.DataFrame({
            "Type": ["Original (public dataset)", "Synthetic / Derived (this project)"],
            "Fields": [ORIGINAL_FIELD_ESTIMATE, synthetic_estimate],
        })
        fig = px.pie(origin_df, names="Type", values="Fields", hole=0.5,
                     color="Type", color_discrete_map={
                         "Original (public dataset)": PRIMARY,
                         "Synthetic / Derived (this project)": ACCENT_2,
                     }, title="Original vs Synthetic/Derived Fields")
        fig.update_layout(height=320, legend=dict(orientation="h", y=-0.15))
        st.plotly_chart(fig, width='stretch', key="dm_provenance_pie")
    with prov_col2:
        st.markdown(f"""
        <div class="provenance-box">
        <b>⚠️ Disclosure:</b> trip IDs, source/destination hubs, OSRM benchmark distance/time,
        actual distance/time and cutoff/segment fields are from the <b>public XpressBees dataset</b>.
        All commercial, cost, fuel, CO₂, hub-workload and risk-related fields are
        <b>synthetic / derived</b> for this WAI assignment.<br><br>
        Cost, fuel, delay-cost and CO₂ figures are <b>modelling estimates</b> — not audited
        XpressBees financials or emissions data.
        </div>
        """, unsafe_allow_html=True)
    ...
    subsection("Analytical Pipeline — Raw Data to Dashboard")
    flow_diagram(
        ["📥 Raw\nData", "🧹 Preparation", "⚙️ Feature\nEngineering", "📊 Analytics",
         "🤖 AI / ML", "⚠️ Risk\nEngine", "🧭 Decision\nEngine", "🗼 Dashboard"],
        per_row=4,
    )
    with st.expander("📋 Data preparation steps (detail)"):
        st.markdown("""
    ...
    # ---- Predictors vs leakage fields (chip visual) -----------------------
    col_a, col_b = st.columns(2)
    with col_a:
        subsection("✅ Predictors Used (Pre-Trip / At-Dispatch)")
        st.caption("Target: `delayed_flag`")
        chip_row(core.SAFE_PREDICTORS_NUMERIC + core.SAFE_PREDICTORS_CATEGORICAL, color=POSITIVE)
    with col_b:
        subsection("🚫 Excluded — Post-Outcome Leakage Fields")
        st.caption("Known only *after* trip completion — never used as predictors.")
        chip_row(core.POST_OUTCOME_LEAKAGE_COLUMNS, color=NEGATIVE)
    ...
    model_cards = [
        (m1, ACCENT, "🎯 Delay Risk", "Classification",
         "Logistic Regression, Decision Tree, Random Forest, Gradient Boosting",
         "Best model chosen on **recall** (a missed high-risk trip is costly), ROC-AUC as tiebreaker."),
        (m2, ACCENT_2, "⏱️ Time Deviation", "Regression",
         "Linear Regression, Random Forest, Gradient Boosting",
         "Simplest model preferred unless a more complex one improves R² by > 0.03."),
        (m3, PRIMARY, "🧩 Trip Segmentation", "Clustering",
         "K-Means, k chosen via silhouette score",
         "Cluster labels derived programmatically from each cluster's z-scored profile — never hardcoded."),
    ]
    for col, color, title, kind, algos, rule in model_cards:
        with col:
            st.markdown(f"""
            <div style="background:white;border-top:5px solid {color};border-radius:8px;
                        padding:0.8rem 1rem;height:100%;box-shadow:0 1px 4px rgba(11,37,69,0.10);">
                <div style="font-weight:750;color:{PRIMARY};font-size:0.98rem;">{title}</div>
                <div style="font-size:0.75rem;color:{color};font-weight:700;text-transform:uppercase;
                            letter-spacing:0.03em;margin-bottom:0.4rem;">{kind}</div>
                <div style="font-size:0.8rem;color:#33465E;"><b>Candidates:</b> {algos}</div>
                <div style="font-size:0.78rem;color:#5B6B82;margin-top:0.4rem;">{rule}</div>
            </div>
            """, unsafe_allow_html=True)
    st.caption("Train/test split: 75/25 (stratified for classification), fixed seed. Models train once "
               "per dataset and are cached — changing dashboard filters never retrains them.")

    st.markdown("<br>", unsafe_allow_html=True)

    # ---- Risk engine band visual -------------------------------------------
    subsection("Risk Engine — Score to Level Mapping")
    st.caption("`risk_probability` (0–1) → `risk_score` (0–100) → `risk_level`")
    risk_labels = getattr(core, "RISK_LABELS", ["Low", "Medium", "High", "Critical"])
    risk_ranges = ["0–30", "31–60", "61–80", "81–100"]
    band_cols = st.columns(len(risk_labels))
    for bc, lbl, rng in zip(band_cols, risk_labels, risk_ranges):
        color = RISK_COLOR_MAP.get(lbl, ACCENT)
        with bc:
            st.markdown(f"""
            <div style="background:{color};color:white;text-align:center;border-radius:8px;
                        padding:0.7rem 0.4rem;font-weight:700;">
                {lbl}<div style="font-size:0.75rem;font-weight:500;opacity:0.9;">{rng}</div>
            </div>
            """, unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # ---- Assumptions / limitations / ethics cards --------------------------
    subsection("Assumptions, Limitations & Ethics")
    a1, a2, a3 = st.columns(3)
    with a1:
        st.markdown(f"""
        <div style="background:#FFF4E5;border-left:5px solid {WARNING};border-radius:8px;
                    padding:0.8rem 1rem;height:100%;">
        <b>⚠️ Key Assumptions</b>
        <ul style="font-size:0.8rem;padding-left:1.1rem;margin-top:0.4rem;">
            <li>Cost / fuel / CO₂ are illustrative estimates, not audited figures</li>
            <li>Scenario Simulator results are directional, not commitments</li>
    ...
    with a2:
        st.markdown(f"""
        <div style="background:#EEF3FA;border-left:5px solid {ACCENT};border-radius:8px;
                    padding:0.8rem 1rem;height:100%;">
        <b>📉 Limitations</b>
        <ul style="font-size:0.8rem;padding-left:1.1rem;margin-top:0.4rem;">
            <li>Limited historical window — seasonality reads are indicative</li>
            <li>Sensitive to synthetic-data generation assumptions</li>
            <li>Clustering/classification quality depends on data coverage</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
    with a3:
        st.markdown(f"""
        <div style="background:#EAF7F0;border-left:5px solid {POSITIVE};border-radius:8px;
                    padding:0.8rem 1rem;height:100%;">
        <b>🛡️ Ethical Considerations</b>
        <ul style="font-size:0.8rem;padding-left:1.1rem;margin-top:0.4rem;">
            <li>No PII displayed or required</li>
            <li>Synthetic/derived data disclosed throughout</li>
            <li>Recommendations are rule-based &amp; explainable, not black-box</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.dataframe(full_df.head(20), width='stretch', height=320)
...
st.markdown("---")
st.caption(
    "XpressBees Logistics Intelligence System — Built for IIM Ranchi Executive MBA WAI Project | "
    "Educational use only. Cost, fuel and CO₂ figures are modelling estimates, not audited XpressBees data."
)
