...
core.py
=======
XpressBees Logistics Intelligence System — Analytical Engine
...
- The base ~24 operational fields (trip_creation_time, route_type, source/
  ...
  from the public XpressBees logistics dataset.
- All commercial, cost, fuel, CO2, hub-workload, risk and several
  ...
  audited XpressBees figures. This is disclosed in the app and README.
"""
...
def load_data(path: str = "data.csv") -> pd.DataFrame:
    """Load the primary XpressBees WAI dataset. Fails clearly if missing."""
    try:
        df = pd.read_csv(path)
    except FileNotFoundError as exc:
        raise FileNotFoundError(
            f"Expected dataset '{path}' was not found at the application "
            f"root. Place the enriched XpressBees dataset (~5,000 rows, "
...
def run_scenario_simulation(df: pd.DataFrame, scenario: str, params: dict) -> dict:
    ...
    represent an actual XpressBees operational commitment.
    """
