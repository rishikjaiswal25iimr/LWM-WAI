...
**Delhivery Logistics Intelligence System — An AI-Enabled Executive Control Tower for Delivery Risk, Cost and Sustainability Management**

## 2. Business Context

**Delhivery** is one of India's largest third-party logistics (3PL) providers, operating a
...
> **How can Delhivery use AI and logistics analytics to identify delivery-risk and route
...
- The **original ~24 Delhivery operational fields** (trip creation time, route type,
  source/destination centers, OSRM distance/time, actual distance/time, cutoff and
  segment-level fields) come from the **public Delhivery logistics dataset**.
...
> **This project uses a publicly sourced Delhivery operational dataset enriched with
...
> must not be interpreted as confidential or audited Delhivery figures.**
...
README.md         # This file
data.csv          # Enriched Delhivery dataset (~5,000 rows, 72 columns)
```
...
- Cost, fuel and CO₂ fields are illustrative modelling estimates, not Delhivery's actual
...
- Synthetic/derived data is disclosed at every relevant point in the application and
  documentation — never presented as confidential Delhivery data.
